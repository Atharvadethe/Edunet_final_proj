...
def get_encoders(df, cat_features):
    ...
    return {col: LabelEncoder().fit(df[col]) for col in cat_features}

@st.cache_resource(show_spinner=False)
def train_model(X_train, y_train):
    """Train a pipeline with scaling and Random Forest model."""
    pipeline = Pipeline([
...
Use the form below to get instant predictions and insights for sustainable farming.
""")

def main():
    DATA_PATH = r'D:\PYTHON\Edunet2.0\FInal_proj\archive\crop_yield.csv'
    ...
    encoders = get_encoders(df, cat_features)
    ...
    if st.button('Predict Crop Yield'):
        try:
            ...
            input_data = np.array([[crop_enc, crop_year, season_enc, state_enc, area, rainfall, fertilizer, pesticide]])
